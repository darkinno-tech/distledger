...
| **文档版本** | 1.0（**文档自身**的版本，与发布版本无关——本库发布版本只有 `v0.1.0`） |
| **状态** | 设计阶段（Design / RFC） |
| **日期** | 2026-09-14 |
| **许可** | MIT |
...
| **G3** | 四条不变量恒成立 | property-based + 随机事件序列测试，10 万次随机序列零违反 |
...
| `reverse_of` | BIGINT | 冲正时指向原佣金单（退款冲正工序引入；在此之前的实现不含该字段，避免恒为零的空壳字段） |
...
> **`WITHDRAWN` 已移除（ADR-044）。** 原设计里 `SETTLED ──提现──> WITHDRAWN`，
...
| ~~`WITHDRAWN`~~ | ~~2~~ | **已移除**。数值 2 被**退役而非复用**：状态以整数持久化，重编号会让已有数据库读错自己的行 |
| `REVERSED` | 3 | 已冲正 |
| `FROZEN` | 4 | 风控冻结 |
| `VOID` | 5 | 作废 |
...
| ⏳ `BindType` | `首次点击` | 归因方式（目前固定为首位优先，不可配） | 防抢客优先于灵活 |
...
| **I4** | 提现单状态与冻结桶、与自身流水三者一致：冻结额 == 未结束提现之和；已打款有打款流水、已驳斥有退回流水 | 标记已打款却没动钱 / 钱卡在冻结桶里 |

### 12.2 测试策略（**先写测试，再写实现**）

1. **Property-based**：随机生成 10 万条事件序列（支付/收货/退款/部分退/重复投递/乱序），断言 I1–I4 恒成立
...
发布版本只有 **v0.1.0** 一个 tag，包含下表中**全部已交付**的内容。
下方的序号是**工作包的先后顺序**（先做内核，再做退款与风控，再做 SQL 存储，再做提现），
**不是版本号**；未交付的项也没有版本号，因为还没有发布计划。

| # | 状态 | 范围 | 验收 |
|---|---|---|---|
| 1 | ✅ 已交付 | 内存 store + 关系链 + 归因 + 多级分佣 + 幂等键 + 冻结快照 + `Maintain` 结算 + I1/I2 自检 | `examples/01` 可跑；`-race` 全绿 |
| 2 | ✅ 已交付 | 退款冲正（整单/明细/部分、逐批累加）+ 风控冻结/解冻/没收 + 不变量 I3 | 退款属性测试通过；三方对账恒成立 |
| 3 | ✅ 已交付 | 可移植 SQL 内核（`store/sql`）+ MySQL / PostgreSQL / SQLite 三个方言 + `Migrate` | 多实例生产可用；重试契约首次被真实数据库执行 |
| 4 | ✅ 已交付 | 提现链路 + `PayoutChannel` + `ManualChannel` + 不变量 I4 + 欠款策略 | `examples/04` 可跑；四个后端通过 |
| 5 | ✅ 已交付 | 集合式对账（`Reconciler`）、社区文件与 CI | 根目录测试不需要数据库；两条硬规则由 CI 强制 |
| 6 | ⏳ 未开工 | 可观测性钩子（指标、埋点）+ `examples` 补齐 02 / 03 / 05 | 接入自诊可用 |
| 7 | ⏳ 未开工 | 规则层扩展（等级费率、SKU 级覆盖） | 规则可插拔验证 |
| 8 | ⏳ 未开工 | 异步打款通道与代扣税 | 见 ADR-043、§11 |

> **仍未实现的两项**：异步打款通道（结果在调用时未知，需要一个本库还没有的状态与配套对账）
> 与代扣税（schema 留了 `tax_amount` 列，无代码写入）。二者都在 PRD 里明确排除在本次范围外，
> 而不是「预留成空壳」——设了却不生效的配置比缺少功能更危险（ADR-022）。

> **API 稳定性承诺**：本库**尚未承诺稳定 API**（目前只有一个版本 `v0.1.0`）。
> 破坏性变更随时可能发生，但**必须**写进 `CHANGELOG.md`。做出稳定承诺之后，将改为遵循语义化版本。
...
├── risk.go                   # 风控：冻结 / 解冻 / 没收
├── withdraw.go               # 提现状态机与模型
├── payout.go                 # 提现引擎与 PayoutChannel
├── selfcheck.go              # 不变量 I1 / I2 / I3 / I4 校验
│
├── internal/safemath/        # 128 位乘除与溢出检测原语
│
├── store/                    # 存储层：端口在根包，实现可替换
│   ├── memory/               # 内存实现（事务回滚 / 键集分页 / 到期索引 / 租户登记）
│   └── sql/                  # 可移植 SQL 内核（含集合式对账），通过 Dialect 拼装语句
├── store/mysql/              # 仅提供 Dialect 与 Open
├── store/postgres/           # 同上
├── store/sqlite/             # 同上
│
├── examples/01-quickstart/   # 零依赖可运行示例
├── examples/04-withdraw/     # 出金链路与欠款策略示例
└── docs/
...
- **破坏性变更**：目前允许（尚未承诺稳定 API），须记入 CHANGELOG
...
| **资金正确性责任** | 高 | 四条不变量 + property-based 测试 + 默认保守 + 免责声明 |
...
*本文档为设计阶段 RFC，欢迎在 Issue 中讨论。核心约束（§5 铁律、§3.2 Non-Goals）不接受变更——它们定义了这个库是什么。*
